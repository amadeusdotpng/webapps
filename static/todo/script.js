		...
		if(xhr.readyState == 4 && xhr.status == 200) {
			...
			for(let i = 0; i < lists.length; i++) {
				...
				for(let j = 0; j < list_data.items.length; j++) {
					...
					item_trashcan.src = '/static/todo/assets/whitetrashcan.png';
				...
				list_trashcan.src = '/static/todo/assets/whitetrashcan.png';
...
function toggle(list_id, item_id) {
	const url = `/todo/toggleitem/${list_id}/${item_id}`;
...
function add_list() {
	...
	const url = `/todo/addlist?name=${encodeURIComponent(name)}`;
...
function add_item(list_id) {
	...
	const url = `/todo/additem/${list_id}?name=${name}`
...
function del_item(list_id, item_id) {
	const url = `/todo/delitem/${list_id}/${item_id}`
...
function del_list(list_id) {
	const url = `/todo/dellist/${list_id}`
